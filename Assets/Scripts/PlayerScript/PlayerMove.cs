...
public class PlayerMove : MonoBehaviour
{
    ...
    private float minJumpPower = 6f; // 최소 점프 힘
    private float maxJumpPower = 12f; // 최대 점프 힘
    ...
    [SerializeField] private Vector2 footPostion; // 발 위치
    void Awake()
    {
    ...
    public IEnumerator RigidbodyOn(Collision2D cs, float time)
    {
        ...
        if (cs.rigidbody != null) // 여기에서도 null 체크를 해야 안전
        {
    ...
    private void CheckGroundAndEnemy()
    {
        ...
        canKillEnemy = Physics2D.OverlapBox(footPostion, enemyCheckBoxSize, 0, enemyHitBoxLayer); // 발 밑에 적의 머리가 있으면 죽일 수 있음
